...
def inference(generations):
        ...
        for abv_i_idx, x_i_sum in enumerate(complete_gen_sum):
            ...
            for abv_j_idx, x_j_sum in enumerate(complete_gen_sum):
                if abv_i_idx <= abv_j_idx:
                    ...
                    covariance_list.append(0)
            covariance_matrix.append(np.array(covariance_list))
        covariance_matrix = np.array(covariance_matrix)
        covariance_matrix = np.rot90(np.flipud(covariance_matrix)).T + covariance_matrix
    ...
    t_0 = sum((seq_0.T * size_0).T)

    seq_k = generations["Sequence"][-1]
    size_k = generations["Size"][-1]
    t_k = sum((seq_k.T * size_k).T)

    for idx, x_i in enumerate(part_2b_summation):
        part_1a = covariance_int
        part_1b = 1
        part_2a = t_k[idx] - t_0[idx]
        part_2b = x_i
